...
The DC-Agent HPC system now includes automatic pre-download functionality for JSC clusters, which don't have internet access on compute nodes. This ensures that datasets and models are downloaded on the login node before jobs are submitted to compute nodes.
...
This feature ensures that the DC-Agent HPC system works seamlessly on JSC clusters while maintaining the same user experience across all supported clusters.
